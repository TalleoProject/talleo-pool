...
function handleServerRequest(request, response) {
    ...
    switch(urlParts.pathname){
        ...
        case '/get_blocks':
            ...
            break;
        case '/get_solver':
            handleGetSolver(urlParts, response);
            break;
...
async function getBlocksChartData() {
    ...
    var promise = new Promise((resolve, reject) => {
        ...
        redisClient.zrevrange(config.coin + ':blocks:matured', 0, maxBlocks, 'WITHSCORES', function(err, result) {
            ...
            resolve({blocks: blocksCount});
        });
    });
    return await promise;
}

async function getPaymentsChartData(address) {
    if (!config.charts.payments || !config.charts.payments.enabled || !config.charts.payments.days) {
        return new Promise({});
    }
    ...
    var promise = new Promise((resolve, reject) => {
        var chartDays = config.charts.payments.days;
        ...
        redisClient.zrangebyscore(config.coin + ":payments:" + address, beginAtTimestamp, Date.now() / 1000, "WITHSCORES", function(error, results) {
            if (error) {
                reject(error);
            }
            ...
            for (var i = 0; i < results.length; i += 2) {
                 var date = utils.dateFormat(new Date(results[i+1] * 1000), 'yyyy-mm-dd');
                 if (chartDays === 1) date = utils.dateFormat(new Date(results[i+1] * 1000), 'yyyy-mm-dd HH:00');
                 var amount = results[i].split(':')[1];
                 var fee = results[i].split(':')[2];
                 if (!paymentsAmount[date]) paymentsAmount[date] = 0;
                 paymentsAmount[date] = paymentsAmount[date] + parseInt(amount);
                 if (!feesAmount[date]) feesAmount[date] = 0;
                 feesAmount[date] = feesAmount[date] + parseInt(fee);
            }
            resolve({payments: paymentsAmount, fees: feesAmount});
        });
    });
    return await promise;
}
    ...
    async.parallel({
        ...
        config: function(callback){
            callback(null, {
                ...
                paymentsChartEnabled: (config.charts.payments && config.charts.payments.enabled),
                paymentsChartDays: config.charts.payments && config.charts.payments.days ? config.charts.payments.days : null
            });
        },
        charts: function (callback) {
            // Get enabled charts data
            charts.getPoolChartsData(async function(error, data) {
                if (error) {
                    callback(error, data);
                    return;
                }
                ...
                // Blocks chart
                var blocksData = await getBlocksChartData();
                if (blocksData && blocksData.blocks) data.blocks = blocksData.blocks;

                // Payments chart
                var paymentsData = await getPaymentsChartData();
                if (paymentsData) {
                    if (paymentsData.payments) data.payments = paymentsData.payments;
                    if (paymentsData.fees) data.fees = paymentsData.fees;
                }
                callback(error, data);
            });
        },
...
function convertPayments(payments) {
    var res = [];
    for (var i in payments.payments) {
       res.push([new Date(i).getTime() / 1000, payments.payments[i]]);
    }
    return res;
}
...
function handleMinerStats(urlParts, response){
    ...
    redisClient.multi([
        ...
        charts.getUserChartsData(address, replies[1], async function(error, chartsData) {
            var paymentsTotal = await getPaymentsChartData(address);
            var res = {
                stats: stats,
                payments: replies[1],
                charts: chartsData,
            ...
            if (paymentsTotal.payments) {
                res['charts']['paymentsTotal'] = convertPayments(paymentsTotal);
            }
...
function handleGetSolver(urlParts, response){
    redisClient.get(config.coin + ':solver:round' + urlParts.query.height, function(error, result){
        var reply;

        if (error){
            reply = JSON.stringify({height: urlParts.query.height, error: 'query failed'});
        } else {
            reply = JSON.stringify({height: urlParts.query.height, solver: result});
        }
