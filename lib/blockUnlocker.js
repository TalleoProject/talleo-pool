    ...
    async.waterfall([
        ...
        function(blocks, callback){
            ...
            blocks.forEach(function(block){
                ...
                orphanCommands.push(['del', config.coin + ':solver:round' + block.height]);
        ...
        function(blocks, callback){
            ...
            blocks.forEach(function(block){
                ...
                var donationPercent = 0.00;
                var solverPercent = (config.blockUnlocker.solverReward || 0) / 100;
                var solver = "";
                ...
                if (Object.keys(donations).length) {
                    for(var wallet in donations) {
                        ...
                        log('info', logSystem, 'Block %d donation to %s as %d percent of reward: %d', [block.height, wallet, percent * 100, payments[wallet]]);
                    }
                }

                var reward = Math.round(block.reward - (block.reward * (feePercent + donationPercent + solverPercent)));
                var poolReward = Math.round(block.reward * feePercent);
                var donationReward = Math.round(block.reward * donationPercent);
                var solverReward = Math.round(block.reward * solverPercent);

                redisClient.get(config.coin + ':solver:round' + block.height, function(error, result){
                    if (error){
                ...
                log('info', logSystem, 'Unlocked %d block with reward %d, pool fee %d and donation fee %d. Miners reward: %d, solver reward: %d', [block.height, block.reward, poolReward, donationReward, reward, solverReward]);
