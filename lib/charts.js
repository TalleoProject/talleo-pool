...
function getPoolHashrate(callback) {
    getPoolStats(function(error, stats) {
        callback(error, (stats && stats.pool) ? Math.round(stats.pool.hashrate) : null);
    });
}

function getPoolWorkers(callback) {
    getPoolStats(function(error, stats) {
        callback(error, (stats && stats.pool) ? stats.pool.workers : null);
    });
}

function getPoolMiners(callback) {
    getPoolStats(function(error, stats) {
        callback(error, (stats && stats.pool) ? stats.pool.miners : null);
    });
}

function getNetworkDifficulty(callback) {
    getPoolStats(function(error, stats) {
        callback(error, (stats && stats.pool) ? stats.network.difficulty : null);
    });
}
...
function getCoinPrice(callback) {
    apiInterfaces.jsonHttpRequest('api.cryptonator.com', 443, true, '', function(error, response) {
